...
extern "C" _declspec(dllexport)HANDLE AGSetInfo(int iCountryCode, unsigned char cPartition, unsigned short iServerCode, char* szServerVersion, char* szServerName, int ServerType, callfunc callbackfunc)
{
	...
	BOOL bIsGSServer = GetPrivateProfileInt("SETUP", "IsCastleSiegeGS", 1, ".\\WzAG.ini");

	char msb[256] = {0};

	if ( bIsGSServer == TRUE )
	{
		GetPrivateProfileString("SETUP", "MonsterSetBaseForGSCS", "Monsters\\MonsterSetBase_CS.txt", msb, 40, ".\\WzAG.ini");
	}
	...
	strcat(szFileName[29], "EventBags\\eventitembag2.txt");
	strcat(szFileName[30], "EventBags\\eventitembag3.txt");
	strcat(szFileName[31], "EventBags\\eventitembag4.txt");
	strcat(szFileName[32], "EventBags\\eventitembag5.txt");
	strcat(szFileName[33], "EventBags\\eventitembag6.txt");
	strcat(szFileName[34], "EventBags\\eventitembag7.txt");
	strcat(szFileName[35], "EventBags\\eventitembag8.txt");
	strcat(szFileName[36], "EventBags\\eventitembag9.txt");
