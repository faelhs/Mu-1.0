...
void ReadyCashShop()
{
	ShopL.ShopCont = GetPrivateProfileInt("ShoppingExt","MaxShop",1,CFG_GAMESERVER);
//	HookThis((DWORD)&BuyRequest,0x4026B2);
//	func.HookProc(0x4EFFA0, (DWORD)(&ShopDataLoad));
	func.HookThis((DWORD)&ShopDataLoad,0x56E671);
	LogSystem::DKLog("Cash Shop", "Inciado");
}
...
void CShop::Init()
{
	...
	LogSystem::DKLog("Cash Shop", "Inciado Init");
...
BOOL CShop::LoadShopItem(char* Filename)
{
	...
	LogSystem::DKLog("Cash Shop", "LoadItem");
	FILE *file;
	...
	while (!feof(file))
	{
		...
		if(Buff[0] ==';' || Buff[0] =='/' || strlen(Buff) < 1 || Buff[0] == '	' || Buff[0] == ' ')
		{
			continue;
		}
...
BOOL CShop::InsertItem(int Type, int Index, int Level, int Dur, BYTE Opt1, BYTE Opt2, BYTE Opt3, BYTE ExcOpt,int BuyMoney)
{
	...
	LogSystem::DKLog("Cash Shop", "InsertItem");
	int Temp = Type * 32 + Index;
...
int CShop::InventoryMapCheck(int X, int Y, int Width, int Height)
{
	...
	LogSystem::DKLog("Cash Shop", "MapChek");
...
void BuyRequest(PMSG_BUYREQUEST* lpMsg, int aIndex)
{
	...
	{
		if (Custom[aIndex].mCash < (int)(ShopC[tShop].Cash[lpMsg->Pos].value))
		{
		...
		else
		{
			...
			if (InsertItem == TRUE)
			{
				...
				pResult.Result = gObjShopBuyInventoryInsertItem(aIndex, ShopC[tShop].Item[lpMsg->Pos]);

				if (pResult.Result != (-1))
				{
					ItemByteConvert(pResult.ItemInfo, ShopC[tShop].Item[lpMsg->Pos]);
					Custom[aIndex].mCash -= ShopC[tShop].Cash[lpMsg->Pos].value;

					if (Custom[aIndex].mCash < 0)
					{
						Custom[aIndex].mCash = 0;
					}
					Manager.ExecFormat("UPDATE MEMB_INFO set Gold = '%d' where AccountID = '%s'", Custom[aIndex].mCash, lpObj->AccountID);
					MsgOutput(aIndex, "Item comprado, valor: %d moedas", ShopC[tShop].Cash[lpMsg->Pos].value);
					MsgOutput(aIndex, "Você ainda possui: %d moedas", Custom[aIndex].mCash);
...
BOOL ShopDataLoad()
{
	...
	LogSystem::DKLog("Cash Shop", "DataLoad");
	LoadShopData();
	...
	ShopC[14].LoadShopItem("..\\Data\\CashShop\\CashShop1.txt");
	ShopC[15].LoadShopItem("..\\Data\\CashShop\\CashShop1.txt");
	ShopC[16].LoadShopItem("..\\Data\\CashShop\\CashShop1.txt");
	*/
